...
export default function TvPacketList() {
  ...
  // load TV packets on mount (same as PacketList but hits /api/packets/tv)
  useEffect(() => {
    const ac = new AbortController();
    fetch(`${API}/api/packets/tv`, { signal: ac.signal })
      .then((r) => {
        if (!r.ok) throw new Error("Failed to fetch TV packets");
        return r.json();
      })
      .then((packetsData) => {
  ...
  return (
    <>
      <div className="title-bar">
        <h1 className="hero-title">Internet & TV Packages</h1>
      </div>

      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "20px" }}>
        {/* Statistics Bar (TV packs only) */}
      ...
      {/* Styles (same as your PacketList) */}
      <style jsx>{`
        .title-bar {
          display: flex;
